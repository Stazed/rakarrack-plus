//
// "$Id: RKR_Input.cxx 11094 2016-01-31 02:49:56Z AlbrechtS $"
//
...
int RKR_Input::shift_position(int p)
{
    return position(p, Fl::event_state(FL_SHIFT) ? mark() : p);
}

int RKR_Input::shift_up_down_position(int p)
{
...
int RKR_Input::kf_lines_up(int repeat_num)
{
    int i = position();
    if (!line_start(i))
    {
...
int RKR_Input::kf_lines_down(int repeat_num)
{
    int i = position();
    if (line_end(i) >= size())
    {
...
int RKR_Input::kf_insert_toggle()
{
    if (readonly())
    {
        fl_beep();
...
int RKR_Input::kf_delete_word_right()
{
    if (readonly())
    {
        fl_beep();
...
int RKR_Input::kf_delete_word_left()
{
    if (readonly())
    {
        fl_beep();
...
int RKR_Input::kf_delete_char_right()
{
    if (readonly())
    {
        fl_beep();
...
int RKR_Input::kf_delete_char_left()
{
    if (readonly())
    {
        fl_beep();
...
int RKR_Input::kf_move_char_left()
{
...
int RKR_Input::kf_move_char_right()
{
    int i = shift_position(position() + 1) + NORMAL_INPUT_MOVE;
    return Fl::option(Fl::OPTION_ARROW_FOCUS) ? i : 1;
}
...
int RKR_Input::kf_move_word_left()
{
...
int RKR_Input::kf_move_word_right()
{
...
int RKR_Input::kf_move_up_and_sol()
{
...
int RKR_Input::kf_move_down_and_eol()
{
...
int RKR_Input::handle(int event)
{
    ...
    case FL_KEYBOARD:
        ...
        if (Fl::event_key() == FL_Tab // Tab key?
                ...
                && ((mark() == 0 && position() == size()) || (position() == 0 && mark() == size())))
        {// while entire field selected?
    ...
    case FL_DND_RELEASE:
        if (dnd_save_focus == this)
        { // if the dragged text comes from the same widget
...
//
// End of "$Id: RKR_Input.cxx 11094 2016-01-31 02:49:56Z AlbrechtS $".
//
