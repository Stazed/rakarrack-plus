                          ...
                          int drive, int eff)
{
    ...
    switch (type + 1)
    {
        ...
        case 11:
        {
            ...
            for (int i = 0; i < nn; i++)
            {
                temps[i] = temps[i] * (ws + 0.5f) * 0.9999f -
                        floorf(0.5f + temps[i] * (ws + 0.5f) * 0.9999f);
            }
            break;
        }
        case 12:
        ...
        case 29: //Valve 2
        {
            ...
            for (int i = 0; i < nn; i++)
            {
                ...
                if (Vg2 <= vfact)
                {
                    Vg2 = vfact / ((-Vg2 / vfact) + 2.0f); //Toward cut-off, behavior is a little different than 2/3 power law
                }
                ...
                if (Vlv2out <= ffact)
                {
                    Vlv2out = ffact / ((-Vlv2out / ffact) + 2.0f); //Then as Vplate decreases, gain decreases until saturation
                }
