...
RKR::changepar(int npar, int value)
{
    ...
    {
        case RKRP_Bypass:
            process_midi_controller_events(MC_Multi_On_Off, EFX_MASTER_ON_OFF);
            break;
        case RKRP_DryWet:
            process_midi_controller_events(MC_Balance_FX,  value);
            break;
        case RKRP_Input:
            process_midi_controller_events(MC_Input_Volume,  value);
            break;
        case RKRP_Output:
        ...
        case RKRP_Preset:   // FIXME this may change DryWet, Input, Output so we need to inform the host...
        {
            ...
            {
                int bank = MIDI_Table[value].bank;
                int preset = MIDI_Table[value].preset + 1;
                process_midi_controller_events(0, bank, preset);  // 0 is CC 0 Bank Select
            }
        }
            break;
        case RKRP_Bank:
            process_midi_controller_events(MC_Bank_Select, value);
            break;
        default:
            return;
    }
